...
This function builds the vector of initial abscissae. If both endpoints of the inputted domain are finite, then the function simple uses those endpoints (plus/minus a small term to protect against closed intervals) as two of the abscissae and samples a third abscissae uniformly on the domain. If either of the endpoints is infinite, then the function iterates over values until it finds one whose first derivative is of the appropriate sign. For example, if D is unbounded on the right, we began at x = 2 and keep moving right until we find an x for which the derivative is negative. Again, once we have two finite abscissae based on the domain endpoints, we sample uniformly on the interval bounded by them to select the third abscissae
...
[mention the check for log concavity]
...
To ensure the validity of the ars function, we ran test_that functions by setting a significance level to 0.05 and making sure that the p-value is greater than the significance level so that the null hypothesis is true. We also ran the ars function based on different densities with different shape parameters/degrees of freedom (normal, exponential, beta((1,1),(2,2)), gamma(shape 1,2), chi-sq with df=2,3, uniform). The p-values for the ars function were all greater than the default significance level. 
